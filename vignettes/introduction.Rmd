...
The function `getSimulationResults()` allows for the simulation of the operating characteristics of a design given one or more effect sizes via the argument `alternative`:

```{r}
getSimulationResults(
  design, alternative = c(0, 0.25), maxNumberOfIterations = 1000, seed = 1234)
```

Note that only 1000 iterations are used in this example, as it only serves for demonstration purposes.
