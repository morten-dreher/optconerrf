...
As an example, we consider an adaptive design with overall type I error rate `alpha=0.025`, early rejection and futility boundaries `alpha1=0.001` and `alpha0=0.5` with a target conditional power `conditionalPower=0.9` at a fixed alternative `delta1=0.5` (note that `useInterimEstimate=FALSE`).
We assume a fixed effect of `deltaLR=0.5` for the calculation of the likelihood ratio (`likelihoodRatioDistribution="fixed"`) and `firstStageInformation=25`.
...
design <- getDesignOptimalConditionalErrorFunction(
  ...
  firstStageInformation = 25, useInterimEstimate = FALSE
)
```
...
The specification of the likelihood ratio under which to calculate the expected second-stage information is given by `distDelta="fixed"` and the corresponding treatment effect is specified by `deltaLR = 0`.

Alternatively, one may calculate the expected second-stage information of the design if it was correctly specified, i.e., for $\Delta=0.5$:
...
A shortcut for the calculation of the expected second-stage information under correct specification is also availabel by omitting the arguments `distDelta` and `deltaLR`, in which case the likelihood ratio specification given in the design is extracted and used for the calculation.
