...
The optimal conditional error function controls for the conditional power of a design at a point alternative `ncp1` or an interim estimate restricted by a minimum `ncp1Min` and possibly a maximum `ncp1Max`.
The mode of action is determined by specifying the logical argument `useInterimEstimate` in addition to the effect size `ncp1` or the restrictions `ncp1Min` and `ncp1Max` to `getDesignOptimalConditionalErrorFunction()`.
...
design <- getDesignOptimalConditionalErrorFunction(
  ...
  firstStageInformation = 4, useInterimEstimate = FALSE
)
print(design)
```
...
getExpectedSecondStageInformation(
  design = design, distDelta = "fixed", deltaLR = 0.5, delta1 = 0.5
)
```
