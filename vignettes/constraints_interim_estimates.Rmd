...
Alternatively, the constraints may be directly specified on the conditional error function as $\alpha_{2,\min}$ and $\alpha_{2,\max}$ due to

$$
\alpha_{2,\min} = \Phi(\Phi^{-1}(CP)- \Delta_1\sqrt{I_{2,\max}})
$$
and
$$
\alpha_{2,\max} = \Phi(\Phi^{-1}(CP)- \Delta_1\sqrt{I_{2,\min}}).
$$
...
Plugging in $I_{2, \max} = 85\cdot 2$ and $I_{2,\min} = 85/2$, we get $\alpha_{2,\min} \approx 0.02396$ and $\alpha_{2,\max} \approx 0.36382$ and can create the same design, now using `minimumConditionalError` and `maximumConditionalError` for the constraints:
...
Comparing the expected second-stage information of the designs for $\Delta=0.25$ yields essentially the same values, with a minor difference resulting from rounding of $\alpha_{2,\min}$ and $\alpha_{2,\max}$. 
...
However, when using a fixed effect size $\Delta_1$ and constant conditional power $CP$, the scales have a one-to-one correspondence (as seen above in the formulas for $\alpha_{2,\min}$ and $\alpha_{2,\max}$) and supplying one of them is sufficient. 

Note that the specification of inappropriate constraints can make it impossible to fully exhaust (and not exceed) the $\alpha$ level, i.e., find a level constant for the design.
(As an example, consider a design with $\alpha=0.025$, $\alpha_1=0$ and $\alpha_0=1$. Since the full $\alpha$ level is to be spent in the second stage, it follows from the level condition (see `?getLevelConstant`) that $0.025 = \int_0^1 \alpha_2(p_1)dp_1$. The exemplary constraint $\alpha_{2,\max}=0.01$ is inappropriate since when applying it, $\int_0^1 \min(0.01, \alpha_2(p_1))dp_1 \leq \int_0^1 0.01 dp_1 = 0.01 \neq 0.025$ and the level condition cannot be fulfilled.)
...
Note that for a design with an early efficacy boundary $\alpha_1>0$, an upper limit for the effect is always implicitly given by $\Delta_{1,\max} = \Phi^{-1}(1-\alpha_1)/\sqrt{I_1}$, which is $\Phi^{-1}(1-0.000158)/\sqrt{85}=0.390672$ in our example.
...
$$
\alpha_{2,\min}(p_1) = \Phi(\Phi^{-1}(CP)- \tilde{\Delta}_1(p_1)\sqrt{I_{2,\max}})
$$
and

$$
\alpha_{2,\max}(p_1) = \Phi(\Phi^{-1}(CP)- \tilde{\Delta}_1(p_1)\sqrt{I_{2,\min}}).
$$

Since $I_{2, \max} \geq I_{2,\min}$, it follows that $\alpha_{2,\min}(p_1) \leq \alpha_{2,\max}(p_1)$.
...
As the smallest effect size corresponds to the largest second-stage information, we can plug in the smallest effect in the formula for $\alpha_{2,\min}(p_1)$ and achieve a constant constraint.
If an upper limit for the effect size is provided, either via $\Delta_{1,\max}$ or at $p_1 = \alpha_1$, the same shortcut is applicable for $\alpha_{2,\max}(p_1)$.
...
Thus, the conditional power function should be decreasing (or at least non-increasing) in the first-stage p-value. Note that if a conditional power function is used, the calculated conditional error function is not necessarily the optimal conditional error function anymore. However, we assume that it is still a sufficiently efficient option.
...
$$
\alpha_{2,\min}(p_1) = \Phi(\Phi^{-1}(CP(p_1)) - \tilde{\Delta}_1(p_1)\sqrt{I_{2,\max}})
$$
and

$$
\alpha_{2,\max}(p_1) = \Phi(\Phi^{-1}(CP(p_1)) - \tilde{\Delta}_1(p_1)\sqrt{I_{2,\min}})
$$
